...
const InnerPublicFileUploadPage: React.FC = () => {
  ...
  const forceRerender = () => setState((prev) => {
    let newValue = Math.random();
    while (prev._forceRerender === newValue) // 중복된 값 방지
  ...
  const onDragLeave = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    const x = e.clientX, y = e.clientY, currentHoveredElement = document.elementFromPoint(x, y);
  ...
  const onClipboardPaste = (event: DocumentEventMap["paste"]) => {
    ignoreEvent(event);
    setState((prev) => ({ ...prev, isMouseOnDragBox: false }));
  ...
  const onSubmit = () => {
    ...
    uploadPublicFileMutation.mutate(
  ...
  }, [fileInputRef.current, fileDragBoxRef.current, state.isMouseOnDragBox]);

  const selectedFile = fileInputRef.current?.files?.length && fileInputRef.current.files[0] || null;

  return <Stack spacing={2} sx={{ flexGrow: 1, width: "100%", minHeight: "100%" }}>
    <Typography variant="h5" gutterBottom>File &gt; PublicFile &gt; 새 파일 업로드</Typography>
    <Input inputRef={fileInputRef} type="file" name="file" sx={{ display: "none" }} />
    <Button variant="outlined" onClick={onFileSelectButtonClick} startIcon={<PermMedia />}>파일 선택</Button>
    <Box
      ref={fileDragBoxRef}
    ...
    >
      위의 버튼을 눌러 사진을 선택하거나,<br />
      이 상자에 직접 파일을 드래그 앤 드롭하여 업로드하거나,<br />
...
export const PublicFileUploadPage: React.FC = () => <BackendAdminSignInGuard>
