 ...
 * 로그인합니다.
 */
export const signInWithEmail = (client: ShopAPIClient) => (data: EmailSignInRequest) => {
 ...
 * SNS로 로그인합니다.
 */
export const signInWithSNS = (client: ShopAPIClient) => async (socialSignInInfo: SocialSignInRequest) => {
 ...
 * @returns 로그인 정보
 */
export const retrieveUserRowInfo = (client: ShopAPIClient) => async () => client.get<UserRow>("v1/users/");

/**
 * 노출 중인 모든 상품의 목록을 가져옵니다.
 * @returns 노출 중인 모든 상품의 목록
 */
export const listProducts = (client: ShopAPIClient) => (qs?: ProductListQueryParams) => client.get<Product[]>("v1/products/", { params: qs });

/**
 * 현재 사용자의 장바구니에 담긴 상품의 목록을 가져옵니다.
 * @returns 현재 장바구니 상태
 */
export const retrieveCart = (client: ShopAPIClient) => () => client.get<Order | EmptyObject>("v1/orders/cart/");

/**
 * 장바구니에 상품을 추가합니다.
 * @returns 현재 장바구니 상태
 */
export const appendItemToCart = (client: ShopAPIClient) => (data: CartItemAppendRequest) =>
  client.post<Order, CartItemAppendRequest>("v1/orders/cart/products/", data);
...
export const removeItemFromCart = (client: ShopAPIClient) => (data: { cartProductId: string }) =>
  client.delete<Order>(`v1/orders/cart/products/${data.cartProductId}/`);

/**
 * 단일 상품 즉시 결제를 PortOne에 등록합니다.
 * @returns PortOne에 등록된 주문 정보
 */
export const prepareOneItemOrder = (client: ShopAPIClient) => (data: OneItemOrderRequest) =>
  client.post<Order, OneItemOrderRequest>("v1/orders/single/", data);

/**
 * 고객의 장바구니에 담긴 전체 상품 결제를 PortOne에 등록합니다.
 * @returns PortOne에 등록된 주문 정보
 */
export const prepareCartOrder = (client: ShopAPIClient) => (data: CustomerInfo) => client.post<Order, CustomerInfo>("v1/orders/", data);

/**
 * 고객의 모든 결제 내역을 가져옵니다.
 * @returns 고객의 모든 결제 내역
 */
export const listOrders = (client: ShopAPIClient) => () => client.get<Order[]>("v1/orders/");

/**
 * 결제 완료된 주문 내역에서 특정 상품을 환불 시도합니다.
 */
export const refundOneItemFromOrder = (client: ShopAPIClient) => (data: OneItemRefundRequest) =>
 ...
 */
export const patchOrderOptions = (client: ShopAPIClient) => async (data: OrderOptionsPatchRequest) =>
 ...
 * 후원자 목록을 가져옵니다.
 */
export const listPatrons = (client: ShopAPIClient, year: number) => () => client.get<Patron[]>("v1/ext/patron/", { params: { year } });
