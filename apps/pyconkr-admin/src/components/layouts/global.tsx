...
import { Box, CssBaseline, Divider, IconButton, ListItem, ListItemButton, ListItemIcon, ListItemText, Stack, styled, SvgIcon, Theme, Toolbar, Tooltip, Typography } from '@mui/material';
  ...
  return <Box sx={{ width: "100%", height: "100%" }}>
    ...
    <MiniVariantAppBar position='fixed' open={state.showDrawer}>
      <Toolbar sx={{ justifyContent: 'flex-start', gap: '1rem' }} disableGutters>
        ...
        <Typography component="div" variant="h6">
          <a href="/" style={{ color: 'white', textDecoration: 'none', fontWeight: 'bolder' }}>PyCon Korea Admin</a>
        </Typography>
      </Toolbar>
    </MiniVariantAppBar>
    ...
    <Box sx={{ width: '100%', minHeight: '100%' }}>
      <MiniVariantDrawer variant='permanent' open={state.showDrawer}>
        ...
        <Divider />
        <Stack sx={{ height: '100%' }}>
          {routes.filter(r => !r.hideOnSidebar).map(r => <SidebarItem key={`${r.app}-${r.resource}`} routeInfo={r} />)}
        </Stack>
      </MiniVariantDrawer>
      <PageOuterContainer>
        <Toolbar />
