...
import { Box, Button, CircularProgress, Stack, Table, TableBody, TableCell, TableHead, TableRow } from '@mui/material';
...
const InnerAdminList: React.FC<AdminListProps> = ErrorBoundary.with(
  ...
  Suspense.with(
    ...
    ({ app, resource }) => {
      ...
      return <Stack sx={{ flexGrow: 1, width: "100%", minHeight: "100%" }}>
        <Box>
          ...
          <TableBody>
            {listQuery.data?.map((item) => (
              <TableRow key={item.id}>
                <TableCell><Link to={`/${app}/${resource}/${item.id}`}>{item.id}</Link></TableCell>
                <TableCell><Link to={`/${app}/${resource}/${item.id}`}>{item.str_repr}</Link></TableCell>
